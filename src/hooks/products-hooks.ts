...
import { CreateProductRequest, ListQueryParams } from '@/lib/api/types';
...
export const useCreateProduct = () => {
  return useMutation<any, CreateProductRequest>(
    (data) => productsApiService.createProduct(data)
  );
};

// Update product hook
export const useUpdateProduct = () => {
  return useMutation<any, { id: string | number; data: Partial<CreateProductRequest> }>(
...
export const useDeleteProduct = () => {
  return useMutation<void, string | number>(
    (id) => productsApiService.deleteProduct(id)
  );
};

// Upload product image hook
export const useUploadProductImage = () => {
  return useMutation<{ url: string }, { productId: string | number; file: File }>(
    ({ productId, file }) => productsApiService.uploadProductImage(productId, file)
  );
};
